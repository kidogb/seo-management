...
import styles from './SampleImageList.less';
...
class ImagesTableList extends PureComponent {
  ...
  columns = [
    ...
    },
    {
      ...
      render: (record) => (
        <Button.Group>
          <Tooltip placement="topLeft" title='Xoá ảnh mẫu'><Button type="danger" icon="delete" ghost onClick={() => this.handleRemoveProduct(record.id)} /></Tooltip>
        </Button.Group>
      ),
    }
  ];
  ...
  handleAddProductVariations = (id) => {
    router.push(`/sample/${id}/variations/registration`);
  }
